...
export interface ProjectRow {
  ...
  share_count?: number;
  /** Only populated by listProjects — who it is shared with (first few). */
  shared_with?: Array<{ email: string; name: string | null }> | null;
  ...
  const { rows } = await sql.query<ProjectRow>(
    `select p.*,
       ...
       (select json_agg(x) from (
          select u.email, u.name from project_shares s
          join users u on u.id = s.user_id
          where s.project_id = p.id
          order by s.created_at limit 4) x) as shared_with
     from projects p
