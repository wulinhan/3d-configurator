...
Staging ground for the embeddable, manifest-driven product configurator — the
self-serve version of the eleven hand-written `visualisation/*-configurator.html`
pages. This directory is temporary: it lives here so the work is committed
somewhere while the dedicated repository is set up, and moves out wholesale.
...
```
node tools/build-model.mjs ../visualisation/tap-bar-3-configurator.html \
     demo/models/tap-bar-3.glb --axes y,z,x
```
